...
/* here we go */
class MatchesController extends MX_Controller {
    private $_c_extends;
    private $_mo_extends;
    private $_mi_extends;
    ...
    */
    public function create_controller()
    {
        ...
        if(isset($controller))
        {
            $names = $this->_names($controller, true);
            ...
            else
            {
                ...
                $this->_find_replace['{{CONTROLLER_FILE}}'] = $class_name.'.php';
                ...
                if(write_file($apppath.'controllers/'.$class_name.'.php',$f))
                {
    ...
    */
    ...
    private function _names($str, $isController = false)
    {
        $str = strtolower($str);
        if(strpos($str,'.'))
        {
        ...
        $class_suffix = $this->config->item('controller_suffix');
        return array('file'=>$file, 'class'=>$class_name.($isController ? $class_suffix : ''),'directories'=>$directories);
    }
